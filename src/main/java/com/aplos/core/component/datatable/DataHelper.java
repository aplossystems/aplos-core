...
class DataHelper {
    ...
    void decodeSortRequest(FacesContext context, DataTable table) {
        ...
        Column sortColumn = findColumn(context, table, sortKey);
    ...
    Column findColumn(FacesContext context, DataTable table, String columnId) {
        ...
        if(group != null) {
            ...
            for (UIComponent child : group.getChildren()) {
                ...
                for (UIComponent headerRowChild : headerRow.getChildren()) {
                    ...
                    if (column.getClientId(context).equals(columnId)) {
                        return column;
                    }
                }
            }
        ...
        if(table.isLazy()) {
            ...
            for(String filterName : filterMap.keySet()) {
                ...
                if(!isValueBlank(filterValue)) {
                    ...
                    if( !column.getId().startsWith( "j_idt" ) ) {
                    	filters.put(column.getId() + ":" + filterField, filterValue );
                    } else {
                    	filters.put(filterField.trim(), filterValue);
                    }
                }
            }
    ...
    void decodeSingleSelection(DataTable table, String selection) {
		if(isValueBlank(selection)) {
			table.setLocalSelection(null);
		} else {
			table.setLocalSelection(table.getRowData(selection));
		}
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	void decodeMultipleSelection(DataTable table, String selection) {
		Class<?> clazz = table.getValueExpression("selectionMultiple").getType(FacesContext.getCurrentInstance().getELContext());

		if(isValueBlank(selection)) {
			Object data = Array.newInstance(clazz.getComponentType(), 0);
			table.setLocalSelection(data);
		}
        else {
            ...
            table.setSelectionMultiple(selectionList.toArray((Object[])selectinArray));
		}
	}
    ...
    String resolveField(ValueExpression expression) {
        ...
        if(newValue == null || !(newValue instanceof String)) {
            ...
            return expressionString;                //Remove var
        }
        else {
            String val = (String) newValue;
            if( !val.startsWith( "tableBean." ) ) {
            	return val;
            } else {
